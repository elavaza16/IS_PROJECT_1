...
exports.toggleDuty = async (req, res) => {
  const { on_duty, latitude, longitude } = req.body;
  const user_id = req.user.id;

  try {
    const [volRows] = await db.query(
      'SELECT volunteer_id, status FROM volunteers WHERE user_id = ?', [user_id]
    );
    if (!volRows.length)
      return res.status(404).json({ error: 'Volunteer profile not found.' });

    if (volRows[0].status !== 'active')
      return res.status(403).json({ error: 'Only active volunteers can go on duty.' });

    if (on_duty) {
      // Going ON duty — must refresh GPS coordinates
      if (!latitude || !longitude)
        return res.status(400).json({ error: 'Location is required to go on duty.' });

      await db.query(
        `UPDATE volunteers SET on_duty = 1, latitude = ?, longitude = ?
         WHERE user_id = ?`,
        [latitude, longitude, user_id]
      );
    } else {
      // Going OFF duty — no location needed
      await db.query(
        `UPDATE volunteers SET on_duty = 0 WHERE user_id = ?`,
        [user_id]
      );
    }

    res.json({ message: on_duty ? 'You are now on duty.' : 'You are now off duty.', on_duty });
  } catch (err) {
